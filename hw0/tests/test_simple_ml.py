...
def test_softmax_loss():
    ...
    np.testing.assert_allclose(softmax_loss(Z,y), 2.3025850, rtol=1e-06)
    Z = np.random.randn(y.shape[0], 10)
    np.testing.assert_allclose(softmax_loss(Z,y), 2.7291998, rtol=1e-06)
...
def test_nn_epoch():
    ...
    np.testing.assert_allclose(dW1.reshape(5,10), W1_0-W1, rtol=1e-2, atol=1e-2)
    np.testing.assert_allclose(dW2.reshape(10,3), W2_0-W2, rtol=1e-2, atol=1e-2)
    ...
    np.testing.assert_allclose(loss_err(np.maximum(X@W1,0)@W2, y),
                               (0.19770025, 0.06006667), rtol=1e-2, atol=1e-2)
