...
def compute_gradient_of_variables(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        grad_node = sum(node_to_output_grads_list[node])
        node.grad = grad_node

        if not node.op:
            continue

        grad_inputs = node.op.gradient(grad_node, node)
        for i, node_in in enumerate(node.inputs):
            #compute grad i->j
            if isinstance(grad_inputs, Tensor):
                grad_i_j = grad_inputs
            else:
                grad_i_j = grad_inputs[i]
            if node_in not in node_to_output_grads_list:
                node_to_output_grads_list[node_in] = []
            node_to_output_grads_list[node_in].append(grad_i_j)
...
def topo_sort_dfs(node, visited, topo_order):
    ...
    for node_in in node.inputs:
        topo_sort_dfs(node_in, visited, topo_order)
